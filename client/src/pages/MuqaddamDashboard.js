...
const MuqaddamDashboard = () => {
  ...
  const handleSubmit = async (e, complaintId, complaintImage, complaintLocation, complaintLat, complaintLng, muqaddamImage, muqaddamLocation, muqaddamLat, muqaddamLng, description) => {
    e.preventDefault();
    ...
    formData.append('muqaddamImage', muqaddamImage);
    ...
    try {
      const res = await fetch('http://localhost:5000/api/complaints/submit-cleanup', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
        body: formData,
      });
...
<button 
  onClick={(e) => handleSubmit(
    e, 
    complaint._id, 
    submissionStatus[complaint._id]?.image,  // Mukaddam's uploaded image
    complaint.location,  // Complaint location
    complaint.latitude,  // Complaint latitude
    complaint.longitude,  // Complaint longitude
    submissionStatus[complaint._id]?.location,  // Mukaddam's location
    submissionStatus[complaint._id]?.latitude,  // Mukaddam's latitude
    submissionStatus[complaint._id]?.longitude,  // Mukaddam's longitude
